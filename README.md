...
- **Мод (дескриптор)** — *рекомендуемый*. Мод как самоописываемый пакет (`descriptors/<id>.json`):
  выбираешь мод из каталога (`⟳ Каталог`) **или** даёшь URL форка. При установки **набором**
  (`🧩 Установить набор`) лаунчер автоматически:
  - подтягивает **зависимости** (`Dependence` из ModuleInfo) — замыкание по графу;
  - **показывает конфликты** (`Conflict`) внутри набора (не снимает — решаешь сам);
  - показывает доступные **обновления** (сверка версии-хэша с каталогом, кнопка `⟳`).
  Код и ассеты тянутся content-addressed чанками с Hugging Face (публично, без токена).
  Один мод может иметь несколько **вариантов** (из разных паков) — по умолчанию берётся
  специализированный источник; можно указать конкретный в поле «Источник».
- **Юнит агрегатора** — мод/пак из `sr-mods-aggregator` по `code.manifest` + `assets.manifest`
  + `asset_index`. Можно весь юнит или один мод (`⟳ Список`, `Категория/Имя`; пусто = весь).
- **ZIP** — ссылка на GitHub Release: zip-ассет распаковывается в `Mods` (фикс CP866-имён,
  нормализация по сегменту `Mods/`).
