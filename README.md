...
- **Юнит агрегатора** — мод из `sr-mods-aggregator`: сборка по рецепту (код-трек релиз +
  только нужные ассет-чанки по `assets.manifest.json` + `state/asset_index.json`, дедуп).
