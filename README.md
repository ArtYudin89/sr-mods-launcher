...
> ⚠️ **Если у вас версия v0.22.0 или старее — обновитесь один раз вручную** (скачайте
> `SRModsLauncher.exe` из [Releases](../../releases/latest) и замените им свой файл).
> В тех версиях помощник обновления ломался на путях с русскими буквами
> (`C:\Users\Артём\…`): лаунчер закрывался, а новая версия не вставала. Начиная с v0.23.0
> обновление в один клик работает как задумано.
...
- **Правая панель.** Вкладка **«Игра»** — синхронизация подключённых модов между игрой и
  профилем; вкладка **«Журнал»** — что делает лаунчер: сколько модов какой сборки уже
  установлено (`ПБ «Свободная Бухта»: 130/300 модов установлено`), что качается, что
  обновляется. Журнал написан обычными словами — его можно скопировать или сохранить
  (правая кнопка мыши по журналу) и отправить автору мода, если что-то пошло не так.
  Галочка **«Подробный лог»** добавляет технические подробности и поимённый список модов.
...
Нажмите **«➕ Добавить мод»**. Есть четыре способа:

- **⚡ Быстрый старт** — выбрать сборку целиком: движок и все её моды одной кнопкой. Открыт
  по умолчанию, пока профиль пуст (пустой список модов тоже предлагает выбрать сборку).
...
- **Сборка → Пак → Мод** — выбрать из структуры каталога. Выбрав сборку, укажите, что из
  неё берёте:
  - **★ Вся сборка (движок + все моды)** — то же, что «Быстрый старт»: в профиль лягут две
    строки, движок и моды, их можно убрать по отдельности;
  - **★ Базовые файлы игры (движок + фиксы)** — то, что лежит вне папки `Mods`
    (`Rangers.exe`, `DATA`, `CFG`) плюс фиксы к базе. **Только от одной сборки:** при попытке
    добавить вторую лаунчер откажет — файлы движка перетирают друг друга, а сохранения
    привязаны к базе;
  - **★ Все моды сборки** — моды без файлов движка; моды разных сборок совмещаются свободно.
- **По ссылке** — добавить мод по прямой ссылке.

> Моды, набранные по одному, файлы движка **не несут**. Если вы ставите игру с нуля — начните
> с «Базовых файлов игры»; **«🛡 Совместимость»** предупредит, если базы нет ни в профиле,
> ни на диске. Установленный движок видно в списке отдельной строкой
> «Базовые файлы игры (движок + фиксы)» с состоянием **✅ установлен**.
...
**Передумали брать отдельный мод из сборки?** Отметьте его галочкой и нажмите
**«⋯ Ещё» → «🗑 Убрать из профиля»**. Сборка останется в профиле, а отмеченные моды просто
перестанут ставиться; окно подтверждения прямо говорит, что именно уберётся — отдельные моды
или целая сборка со всем её содержимым. Файлы, уже лежащие в игре, при этом не удаляются.
Вернуть убранное — **«⋯ Ещё» → «↩ Вернуть исключённые»**.
...
их по игре так, как ожидает сама игра. Идёт индикатор прогресса, установку можно **поставить на
паузу** (**⏸ Пауза** → **▶ Продолжить**) или прервать совсем. Пока идёт закачка, лаунчером можно
пользоваться: искать моды, листать список, открывать карточки и настройки — заблокированы только
действия, которые сами меняют файлы.
...
Значок **ⓘ** у строки открывает карточку мода: описание, автор, раздел, **откуда** он установлен
(сборка и паки — основа и фиксы, в порядке установки), ваши теги и заметка, а также **связи** —
что мод требует, для чего он нужен и с чем конфликтует. Карточку можно двигать, менять размер и
держать несколько открытыми, чтобы сравнить моды. У мода, который есть в нескольких сборках,
здесь же переключаются **варианты** — вы сами выбираете, из какой сборки взять мод.

Один и тот же мод нередко лежит и в авторской раздаче, и внутри пака-сборника — с разными
датами. Выбор в карточке **сильнее порядка паков**: даже при установке сборки целиком файлы
мода возьмутся из выбранного пака, а не из того, что накладывается последним. Исправления
(фикс-паки) к выбранному паку при этом применяются как обычно.

Бывает и сложнее: у мода есть версия «своей» сборки под другим именем — например `ShuDomiks`
раздаётся в Original и Universe, а Свободная Бухта даёт `PolDomiks` в ту же папку. Тогда кнопок
в строке три: `PolDomiks · REDUX`, `ShuDomiks · ORIG`, `ShuDomiks · UNI`. Метка и дата в списке
всегда показывают **ту версию, которая реально встанет**.
...
**Мод, который не нужно обновлять.** Если вы правили мод руками или вас устраивает нынешняя
версия — пометьте его: правый клик по строке → **«🔒 Не обновлять»** (сразу для всех выделенных),
кнопка в карточке мода или **«🔒 Больше не предлагать»** в окне с конфликтами. Такой мод не
проверяется и не попадает в «Обновить все», в списке у него состояние **«🔒 не обновляется»**;
снять пометку — там же. Скрытые моды по умолчанию продолжают обновляться — если хотите иначе,
включите в ⚙ Настройках галочку **«Скрытые моды не обновлять»**.
...
Кнопка **⚙** открывает настройки: папка игры, проверка обновления лаунчера, поведение обновления
модов и раздел **«Внешний вид»** — регуляторы **Размер текста / Масштаб / Контраст** (каждый со
сбросом ↺). Тему можно переключать и прямо из шапки кнопкой 🌙/☀. Значки **?** рядом с названиями
разделов — кликабельные подсказки: нажмите, чтобы прочитать, что делает настройка.
...
- **Лаунчер не обновляется сам (закрывается и не возвращается).** Так было в версиях
  до v0.23.0 на путях с русскими буквами — скачайте `SRModsLauncher.exe` из Releases вручную
  один раз, дальше обновление в один клик работает.
- **Надоело каждый раз отвечать на вопрос об обновлении мода.** Пометьте мод
  **🔒 «Не обновлять»** (правый клик по строке) — он перестанет попадать в проверку.
...
Код лаунчера — **GNU GPL v3 или новее**, полный текст в [LICENSE](LICENSE).
Copyright (C) 2026 ArtYudin89.

Что это значит на практике: берёте код или собираете свою сборку лаунчера — открывайте
исходники своей версии на тех же условиях. Исходники релизной сборки лежат прямо в
репозитории (`SRModsLauncher-source.zip` и дерево `webui/`), так что требование GPL об
исходном коде для распространяемых `.exe` выполнено.

Лицензия покрывает **только код лаунчера**. Она **не покрывает** моды, их ассеты и
ресурсы самой игры: моды принадлежат своим авторам, ресурсы игры — правообладателям.
Лаунчер их только скачивает и раскладывает.

## Поддержать

USDT и прочие способы — в [DONATE.md](DONATE.md). Лаунчер бесплатный, без рекламы и
телеметрии; донат ничего не открывает.
