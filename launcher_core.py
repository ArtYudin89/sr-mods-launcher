...
def install_route(relpath):
    ...
    # 0) путь, выводящий за пределы игры, — не контент мода ни при каких условиях.
    # Пути приходят из дескрипторов каталога и из имён записей в zip; zipfile и
    # json не проверяют их сами. Про '..' проверка ниже (п.0b) сработала бы и так,
    # но лишь по совпадению — она про .temp-каталоги агрегатора, и стоит её однажды
    # сузить до '.temp', как побег молча открывается снова.
    # Буква диска отдельно и опаснее: base / 'C:/Windows/x' в pathlib даёт
    # АБСОЛЮТНЫЙ путь — правая часть побеждает целиком, и файл уезжает не «на
    # уровень выше», а куда угодно. Ведущий '/' безобиден: пустой сегмент выше
    # уже отброшен, путь остаётся относительным.
    if any(p == '..' or ntpath.splitdrive(p)[0] for p in parts):
        return (None, None)
    # 0b) staging агрегатора (.temp/.tmp и пр. скрытые сегменты) — не контент мода
